      ...
      private SQLiteConnection db { get; set; }
           ...
           public ConnectionClass()
       {
           ...
           db = new SQLiteConnection(dbPath);
            ...
            db.CreateTable<HangmanQuestion>();
        }
        ...
        public List<HangmanQuestion> GetRandomQuestion()
        {
            try
            {
                ;
                //return db.Query<HangmanQuestion>("SELECT * FROM HangmanQuestions ORDER BY RANDOM() LIMIT 1");
                return db.Query<HangmanQuestion>("SELECT * FROM HangmanQuestions");
            }
            catch (Exception e)
            {
                Console.WriteLine("Error:" + e.Message);
